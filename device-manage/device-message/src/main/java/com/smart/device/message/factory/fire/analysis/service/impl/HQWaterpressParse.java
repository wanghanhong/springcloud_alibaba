...
import com.smart.device.message.parse.fire.waterpress.water.huaqiang.strategy.WaterParseStrategy;
import com.smart.device.message.parse.fire.waterpress.water.huaqiang.strategy.WaterStandar;
 ...
 */
@Service
public class HQWaterpressParse implements FireDeviceParse {

    @Resource
    private WaterParseStrategy waterParseStrategy;
    @Resource
    private WaterStandar waterStandar;
    @Resource
    ...
    @Resource
    private THeartWaterpressSonService tHeartWaterpressSonService;
    @Resource
    ...
    public String parse(String data) {
        ParseContext<WaterEntity> context = new ParseContext<>(waterParseStrategy);
        WaterEntity entity = context.parseEntity(data);
        WaterEntityVo waterEntityVo =  waterParseStrategy.convertVo(entity);
        TAlarmWaterpress alarm = waterStandar.changeToAlarm(waterEntityVo);
        THeartWaterpress heart = waterStandar.changeToTHeart(waterEntityVo);
        if(alarm.getImei() != null){
            ...
            if (exist  !=null && "1".equals(exist)){
                ...
                if(baseVo != null){
                    ...
                    Long id = tHeartWaterpressService.waterpressAdd(heart);
                    THeartWaterpressSon son = heart.getWpSon();
                    if(son != null){
                        son.setId(id);
                        tHeartWaterpressSonService.waterpressSonAdd(son);
                    }
                }
