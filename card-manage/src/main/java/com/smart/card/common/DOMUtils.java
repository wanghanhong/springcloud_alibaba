...
public class DOMUtils {
    ...
    public static List<Map<String, Object>> strToXmlAndPaserXml(String strXml) throws Exception {
        SAXReader reader = new SAXReader();
    ...
    // 遍历解析xml数据
    public static List<Map<String, Object>> paserXml(Document doc) throws Exception {
        ...
        while (it.hasNext()) {
            ...
            while (its.hasNext()) {
                ...
                Iterator<?> its1 = elements.elementIterator();
                if (elements.isTextOnly()) {
                ...
                int c = 0;
                while (its1.hasNext()) {
                    elements1 = (Element) its1.next();
                    Iterator<?> its2 = elements1.elementIterator();
                    if (elements1.isTextOnly()){
                        map1.put(elements1.getName(), elements1.getText());
                        continue;
                    }
                    Map<String, Object> map2 = new HashMap<>();
                    while (its2.hasNext()) {
                        c++;
                        elements2 = (Element) its2.next();
                        Iterator<?> its3 = elements2.elementIterator();
                        if (elements2.isTextOnly()){
                            map2.put(elements2.getName(), elements2.getText());
                            continue;
                        }
                        int cnt = 0;
                        ...
                        while (its3.hasNext()) {
                            cnt ++;
                            elements3 = (Element) its3.next();
                            map3.put(elements3.getName(), elements3.getText());
                        }
                        map2.put(elements2.getName() + cnt, map3);
                    }
                    map1.put(elements1.getName() + c, map2);
                }
                map.put(elements.getName() + count, map1);
            }
            m.put(element.getName() + t, map);
        }
        list.add(m);
        return list;
    }
    ...
    public static <T> List<T> strToXmlAndPaserEntity(String strXml, Class<T> clazz) throws Exception {
        SAXReader reader = new SAXReader();
        Document doc = reader.read(new ByteArrayInputStream(strXml.getBytes(StandardCharsets.UTF_8)));
        return paserXmlEntity(doc, clazz);
    }

    public static <T> List<T> paserXmlEntity(Document doc, Class<T> clazz) throws Exception {
    ...
    public static List<Map<String, Object>> paserXmlAttribute(String xml, String nodeName) {
        String XMLResponse = xml.substring(xml.indexOf("<XMLResponse>"));
        Element em = DOMUtils.parseXml(XMLResponse);
