...
import com.smart.brd.sys.common.service.CacheService;
import com.smart.brd.sys.common.utils.MD5Util;
import com.smart.brd.sys.common.utils.SortUtil;
import com.smart.brd.sys.system.dao.UserMapper;
import com.smart.brd.sys.system.dao.UserRoleMapper;
...
import com.smart.brd.sys.system.service.UserRoleService;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void createUser(HttpServletRequest request,User user) throws Exception {
        UserBean userBean = userTokenService.getUserByToken(request);
        ...
        if (StringUtils.isNotEmpty(user.getMobile())) {
            List<User> users = baseMapper.selectList(new LambdaQueryWrapper<User>().eq(User::getMobile, user.getMobile().trim()));
            if (!users.isEmpty()) {
                throw new FebsException("手机号重复，请重试");
            }
        }
        // 创建用户
    ...
    public void updateUser(HttpServletRequest request,User user) throws Exception {
        ...
        if (StringUtils.isNotEmpty(user.getUsername())) {
            ...
            if (users.size()==1&&!user.getUserId().equals(users.get(0).getUserId())) {
                throw new FebsException("用户名重复，请重试");
            }
        }
        if (StringUtils.isNotEmpty(user.getMobile())) {
    ...
    public Long querySysConfig() {
        ...
        Long time_expire =  Long.parseLong((String)redisTemplate.opsForValue().get(key));
        if (time_expire == null){
