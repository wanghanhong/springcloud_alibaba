package com.smart.video.manage.common.aspect;
...
import com.smart.video.manage.common.usertoken.constant.RedisConst;
...
public class UserIdAdvice {
     ...
     * 切入点，定义所有的控制器
     */
    @Pointcut("execution(public * com.smart.video.manage.controller.*.*(..))||execution(public * com.smart.brd.manage.base.common.test.*.*(..))||execution(public * com.smart.brd.manage.base.screen.controller.*.*(..))")
...
//            throw new SecurityException("Token异常！");
    ...
    private void getAndSetEntity(Long deptId,String deptIds, Object arg) {
        //对于不同的Vo类进行相应的赋值
        if (arg instanceof TVideo) {
            TVideo vo = (TVideo) arg;
            vo.setDeptId(deptId);
